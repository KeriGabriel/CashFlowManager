...
namespace Gabriel_CashFlowManager
{
    ...
    {     
        static void Main(string[] args)
        {
            ...
            ledgers[4] = new Invoice("5896", 1, "On-board nuclear reactor", 10000.95m);
            ...
            void addSalariedEmployeeMenu()
            {
                ...
                Console.WriteLine("\n***Added this employee*** ");
                Console.WriteLine("\n Name: " + firstname + " " + lastname + "\n SSN: " 
                        + SSN + "\n Weekly wage: " + weeklyWage.ToString("c") +"\n");
                index++;
            ...
            for (int i = 0; i < ledgers.Length; i++)
            {
                if (ledgers[i] == null)
                    break;
                if (ledgers[i].ledgerType == IPayable.LedgerType.Invoice.ToString())
                {
                   PartsWeeklyPayout = PartsWeeklyPayout + ledgers[i].PayableAmount;   
                }
                    if (ledgers[i].ledgerType == IPayable.LedgerType.Salaried.ToString())
                {   
                    salariedWeeklyPayout = salariedWeeklyPayout + ledgers[i].PayableAmount;
                }
                if (ledgers[i].ledgerType == IPayable.LedgerType.Hourly.ToString())
                {
